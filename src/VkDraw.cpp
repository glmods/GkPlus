...
struct TonemapPushConstants {
  ...
  float knee;
  float white;
  // The bloom composite (§4.99). Arrays rather than nine scalars, because the shader indexes them
  // in a loop - and an array of a 4-byte scalar has stride 4 under both layout rules, so it keeps
  // the property the comment above this struct is about.
  uint32_t bloom_texture[kBloomLayers];
  uint32_t bloom_blend[kBloomLayers];
  float bloom_intensity[kBloomLayers];
  uint32_t bloom_sampler;
};
static_assert(sizeof(TonemapPushConstants) == 80);

// Must match `BloomPush` in src/shaders/bloom.slang, and here for the same reason the two above are.
// One block for both of that file's passes; `taps` and `direction` are the two fields whose meaning
// differs between them, which the shader spells out.
...
static_assert(sizeof(BloomPushConstants) == 36);
...
// scope - `src/VkDraw.h`, `src/VertexFormat.h`, and the five push blocks above, which are
...
// One image plus its memory and view. Written out once rather than at each of its ten call sites,
// because they differ only in format, size and usage - and a copy-pasted allocation that binds the
// wrong memory to the wrong image is invisible until something reads it.
//
// It was named for the AO pass while that was the only caller; the bloom pass's six images come
// through it too, so the name no longer says AO.
bool CreatePassImage(uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage,
...
bool CreateAoTargets(uint32_t width, uint32_t height) {
  ...
  if (!CreatePassImage(width, height, kAoPositionFormat, kColour, VK_IMAGE_ASPECT_COLOR_BIT,
                     &AoPositionImage, &AoPositionMemory, &AoPositionView) ||
      !CreatePassImage(width, height, kAoNormalFormat, kColour, VK_IMAGE_ASPECT_COLOR_BIT,
                     &AoNormalImage, &AoNormalMemory, &AoNormalView) ||
      !CreatePassImage(width, height, kAoResultFormat, kColour, VK_IMAGE_ASPECT_COLOR_BIT,
                     &AoResultImage, &AoResultMemory, &AoResultView) ||
      !CreatePassImage(width, height, DepthFormat, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
...
//
// See "bloom: three layers over the HDR target" in VkDraw.h for the design and for why each layer
// extracts from the float target itself rather than from the layer above it. Everything here is the
// pass and its knobs; the only other code that knows bloom exists is `RecordTonemap`, which
// composites the finished layers, and the resize path, which rebuilds the images.

// **A layer's height in LINES, absolute rather than a fraction of the frame** - see the note in the
// header. 240/120/60 is a factor of two between neighbours, which is what makes three layers cover
// a useful span: with a sigma of a couple of texels each, they are a 1%, a 2% and a 6% glow.
constexpr uint32_t kBloomLayerHeights[kBloomLayers] = {240, 120, 60};

// The blur kernel's half-width cap, in layer texels. 24 makes the widest kernel 49 fetches, which at
// 240 lines is 4.9 M for the pair of passes and at 60 lines is 300 k. **No default comes near it** -
// the widest asks for 11 - so it is a ceiling on what a REPL session can ask for rather than a
// quality dial, and a radius past it stops widening instead of costing more.
constexpr uint32_t kBloomMaxTaps = 24;

// The extract's box, per axis. Four linear taps cover about eight source texels, which is the
// 1728-line case; a 640x480 frame needs one. See `BloomExtractTaps`.
constexpr uint32_t kBloomMaxExtractTaps = 4;

// The same format the world pass draws into, which is not a coincidence to be tidied away: this
// pass exists to work on over-range light, so a target that could not hold it would defeat the
// feature rather than merely quantise it.
constexpr VkFormat kBloomFormat = kHdrFormat;

// **Off by default**, for the reason every departure in this file is: a default run has to keep the
// residual claim against `GKPLUS_RENDERER=d3d8` true.
bool BloomEnabled = false;
bool BloomEnvRead = false;

// The defaults, and they are a look rather than three arbitrary numbers. Each threshold sits at or
// just above 1.0, which under `render.linear_input` is exactly "brighter than a fully lit opaque
// surface" - so on a frame with no fire, no flare and no over-bright light in it, all three layers
// pass nothing and bloom is invisible. That is the intended baseline: this game's world is mostly
// under 1, and what exceeds it is what should glow.
//
// The three radii are a factor of ~3 apart rather than a factor of 2, because the layer heights
// already carry a factor of 2 - so in each layer's own texels the sigmas come out at 1.4, 2.4 and
// 3.6, all of them cheap and none of them so wide that the truncated kernel shows.
BloomLayer BloomLayers[kBloomLayers] = {
    // Tight: the halo immediately around a bright thing. The strongest, because it is the one that
    // reads as "this surface is emitting" rather than as haze.
    {.threshold = 1.0f, .knee = 0.5f, .radius = 0.006f, .intensity = 0.5f,
     .blend = BloomBlend::Add},
    // Mid: the glow that spreads onto neighbouring geometry.
    {.threshold = 1.2f, .knee = 0.7f, .radius = 0.02f, .intensity = 0.35f,
     .blend = BloomBlend::Add},
    // Wide, and the one that is `Screen` rather than `Add`: a 6%-of-height blur reaches most of the
    // frame, so adding it would raise the whole image's black level - which is the thing that makes
    // a bloom look like fog. Screen puts it only where the frame is not already lit.
    {.threshold = 1.6f, .knee = 1.0f, .radius = 0.06f, .intensity = 0.22f,
     .blend = BloomBlend::Screen},
};

bool BloomPassReady = false;
VkImage BloomImage[kBloomLayers][2] = {};
VkDeviceMemory BloomMemory[kBloomLayers][2] = {};
VkImageView BloomView[kBloomLayers][2] = {};
VkExtent2D BloomExtent[kBloomLayers] = {};
// What the extract was told to downsample from, kept for the report: a radius is only interpretable
// beside the size it is measured in.
uint32_t BloomSourceHeight = 0;
...
VkShaderModule BloomVertexModule = VK_NULL_HANDLE;
VkShaderModule BloomExtractModule = VK_NULL_HANDLE;
VkShaderModule BloomBlurModule = VK_NULL_HANDLE;
// How many layers this frame actually blurred, and whether any did. The tonemap's push is filled
// from the second of these rather than from `BloomEnabled`, which is what keeps the composite from
// sampling a layer whose passes did not run - the frame after a knob is switched on, or any frame
// where the colour format is still the 8-bit one.
uint32_t BloomLayersThisFrame = 0;
bool BloomRanThisFrame = false;

void ReadBloomEnvOnce() {
  if (BloomEnvRead) {
    return;
  }
  BloomEnvRead = true;
  char value[16] = {};
  const DWORD len = ::GetEnvironmentVariableA("GKPLUS_VK_BLOOM", value, sizeof(value));
  if (len == 0 || len >= sizeof(value)) {
    return;
  }
  const std::string text(value, len);
  BloomEnabled = !(text == "0" || text == "off" || text == "no");
}

// How many box taps per axis the extract needs, given what it is reading and what it is writing.
//
// One linear fetch averages about two source texels, so the count follows half the ratio. **A single
// tap over a 7-texel footprint is the failure this exists to avoid**, and it does not present as
// aliasing: it presents as individual bright pixels flickering in and out as the camera moves, which
// reads as the effect being broken rather than under-sampled.
uint32_t BloomExtractTaps(uint32_t source_height, uint32_t dest_height) {
  if (dest_height == 0) {
    return 1;
  }
  const uint32_t ratio = (source_height + dest_height - 1) / dest_height;
  return std::clamp((ratio + 1) / 2, 1u, kBloomMaxExtractTaps);
}

// The kernel half-width a sigma needs, in the layer's own texels. Three sigmas covers 99.7% of the
// Gaussian's mass; the shader renormalises by the weights it actually took, so the cap truncates the
// kernel rather than dimming what it blurs.
uint32_t BloomBlurTaps(float sigma) {
  if (!(sigma > 0.0f)) {
    return 1;
  }
  const auto taps = static_cast<uint32_t>(std::ceil(3.0f * sigma));
  return std::clamp(taps, 1u, kBloomMaxTaps);
}

// A layer's sigma in its own texels. `radius` is a fraction of the frame HEIGHT and a layer covers
// the whole frame in `BloomExtent[i].height` texels, so the conversion is exactly that product -
// which is what makes the knob resolution-independent without anything else having to know.
float BloomSigma(uint32_t layer) {
  return BloomLayers[layer].radius * static_cast<float>(BloomExtent[layer].height);
}

void DestroyBloomTargets() {
  for (uint32_t layer = 0; layer < kBloomLayers; ++layer) {
    for (uint32_t buffer = 0; buffer < 2; ++buffer) {
      if (BloomView[layer][buffer] != VK_NULL_HANDLE) {
        vkDestroyImageView(GetDevice(), BloomView[layer][buffer], nullptr);
        BloomView[layer][buffer] = VK_NULL_HANDLE;
      }
      if (BloomImage[layer][buffer] != VK_NULL_HANDLE) {
        vkDestroyImage(GetDevice(), BloomImage[layer][buffer], nullptr);
        BloomImage[layer][buffer] = VK_NULL_HANDLE;
      }
      if (BloomMemory[layer][buffer] != VK_NULL_HANDLE) {
        vkFreeMemory(GetDevice(), BloomMemory[layer][buffer], nullptr);
        BloomMemory[layer][buffer] = VK_NULL_HANDLE;
      }
    }
    BloomExtent[layer] = {};
  }
...
// Six small images at the render extent's aspect ratio. Called from `CreateBloomPass` and again from
// `ResizeDraw`, separately from the pipelines for the reason `CreateAoTargets` is: a resize must not
// rebuild a pipeline, and these pipelines never need rebuilding at all - they write `kBloomFormat`,
// which is a constant, so unlike the world's they are untouched by `render.hdr` and `render.msaa`.
bool CreateBloomTargets(uint32_t width, uint32_t height) {
  DestroyBloomTargets();
  if (width == 0 || height == 0) {
    return false;
  }
  constexpr VkImageUsageFlags kUsage =
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
  for (uint32_t layer = 0; layer < kBloomLayers; ++layer) {
    // **Clamped to the frame rather than assumed smaller than it.** Gunlok runs at 320x240, where
    // the first layer's reference height is the whole frame; without the clamp the extract would be
    // an upsample and the layer would cost more than the image it came from.
    const uint32_t layer_height = (std::min)(kBloomLayerHeights[layer], height);
    // From the height, so the layer has the frame's aspect ratio whatever it is - the same reason
    // `ao_screen_radius` is a fraction of the height and not the width.
    const uint32_t layer_width =
        (std::max)(1u, static_cast<uint32_t>(std::lround(static_cast<double>(width) *
                                                      layer_height / height)));
    for (uint32_t buffer = 0; buffer < 2; ++buffer) {
      if (!CreatePassImage(layer_width, layer_height, kBloomFormat, kUsage,
                           VK_IMAGE_ASPECT_COLOR_BIT, &BloomImage[layer][buffer],
                           &BloomMemory[layer][buffer], &BloomView[layer][buffer])) {
        DestroyBloomTargets();
        return false;
      }
      // Written once here rather than per frame: a bindless slot is a pointer to a view, and these
      // views only change when the extent does - at which point this runs again.
      WriteBindlessView(BloomSlot(layer, buffer),
                        reinterpret_cast<uint64_t>(BloomView[layer][buffer]));
    }
    BloomExtent[layer] = {layer_width, layer_height};
  }
...
void DestroyBloomPass() {
  ...
  DestroyBloomTargets();
  VkPipeline *pipelines[] = {&BloomExtractPipeline, &BloomBlurPipeline};
  for (VkPipeline *pipeline : pipelines) {
    if (*pipeline != VK_NULL_HANDLE) {
      vkDestroyPipeline(GetDevice(), *pipeline, nullptr);
      *pipeline = VK_NULL_HANDLE;
  ...
  if (BloomLayout != VK_NULL_HANDLE) {
    vkDestroyPipelineLayout(GetDevice(), BloomLayout, nullptr);
    BloomLayout = VK_NULL_HANDLE;
  }
  VkShaderModule *modules[] = {&BloomVertexModule, &BloomExtractModule, &BloomBlurModule};
  for (VkShaderModule *module : modules) {
    if (*module != VK_NULL_HANDLE) {
      vkDestroyShaderModule(GetDevice(), *module, nullptr);
      *module = VK_NULL_HANDLE;
...
// The two pipelines differ in exactly one field, so they are built from one function - a
// copy-pasted 60-line pipeline description with one line changed is the shape that goes stale.
bool CreateBloomPipeline(VkShaderModule fragment, VkPipeline *out) {
  VkPipelineShaderStageCreateInfo stages[2] = {};
  stages[0].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  stages[0].stage = VK_SHADER_STAGE_VERTEX_BIT;
  stages[0].module = BloomVertexModule;
  stages[0].pName = "bloom_fullscreen_vertex";
  stages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  stages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
  stages[1].module = fragment;
  stages[1].pName = fragment == BloomExtractModule ? "bloom_extract_fragment"
                                                   : "bloom_blur_fragment";
  ...
  raster.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
  raster.lineWidth = 1.0f;
  // One sample, whatever `render.msaa` says: these targets are the pass's own and are never the
  // world attachment, so the resolve has already happened by the time anything here reads a pixel.
  VkPipelineMultisampleStateCreateInfo multisample = {
      VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO};
  multisample.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;
  VkPipelineDepthStencilStateCreateInfo depth = {
  ...
  // `kBloomFormat` and not the swapchain's or the world's: the layers are this pass's own images, so
  // neither `render.hdr` nor a resize can invalidate these two pipelines.
  VkFormat format = kBloomFormat;
  VkPipelineRenderingCreateInfo rendering = {VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO};
  rendering.colorAttachmentCount = 1;
  ...
  info.pColorBlendState = &blend;
  info.pDynamicState = &dynamic;
  info.layout = BloomLayout;
  return vkCreateGraphicsPipelines(GetDevice(), VK_NULL_HANDLE, 1, &info, nullptr, out) ==
         VK_SUCCESS;
}

bool CreateBloomPass(uint32_t width, uint32_t height) {
  BloomVertexModule = CreateModule(kBloomFullscreenVertexSpv, sizeof(kBloomFullscreenVertexSpv));
  BloomExtractModule = CreateModule(kBloomExtractFragmentSpv, sizeof(kBloomExtractFragmentSpv));
  BloomBlurModule = CreateModule(kBloomBlurFragmentSpv, sizeof(kBloomBlurFragmentSpv));
  if (BloomVertexModule == VK_NULL_HANDLE || BloomExtractModule == VK_NULL_HANDLE ||
      BloomBlurModule == VK_NULL_HANDLE) {
    return false;
  }
  // Both passes read out of the bindless array, exactly as the tonemap and the AO resolve do, so
  // there is no descriptor set of this pass's own to build.
  auto set_layout = reinterpret_cast<VkDescriptorSetLayout>(BindlessDescriptorSetLayout());
  if (set_layout == VK_NULL_HANDLE) {
    return false;
  }
  ...
  layout.pushConstantRangeCount = 1;
  layout.pPushConstantRanges = &range;
  if (vkCreatePipelineLayout(GetDevice(), &layout, nullptr, &BloomLayout) != VK_SUCCESS) {
    return false;
  }
  if (!CreateBloomPipeline(BloomExtractModule, &BloomExtractPipeline) ||
      !CreateBloomPipeline(BloomBlurModule, &BloomBlurPipeline) ||
...
// One of this pass's own images, into or out of being read. Two shapes and no others, which is why
// this takes a bool rather than four stage masks.
//
// **UNDEFINED as the old layout on every transition into an attachment**, on purpose: each pass
// writes every pixel of its target under a DONT_CARE load, so preserving what was there is paying to
// decompress an image about to be overwritten whole. The half that matters - the execution
// dependency - is carried by the stage masks, and those are exact either way.
void BloomBarrier(VkCommandBuffer cmd, VkImage image, bool to_attachment) {
  ...
  if (to_attachment) {
    ...
    // The read this write must wait for is the previous pass's fetch of the same image - the
    // horizontal blur's, on the transition that matters - or the previous frame's, which the frame
    // fence has already covered.
...
// One full-screen triangle into one of this pass's images.
void RecordBloomStep(VkCommandBuffer cmd, VkPipeline pipeline, VkImageView target,
                     VkExtent2D extent, const BloomPushConstants &push) {
  ...
  // DONT_CARE and not CLEAR: every one of these passes writes every pixel of its target
  // unconditionally, so a clear would be a second full write of the image.
  ...
  // **A zero origin, not the world pass's half-pixel one**, for the reason the AO resolve gives:
  // the geometry is a full-screen triangle and the fragment reads its own integer `SV_Position`, so
  // shifting the viewport would move every pixel's idea of which texel it is by half of one.
  ...
  vkCmdPushConstants(cmd, BloomLayout,
...
bool StartDraw(uint32_t width, uint32_t height, uint32_t colour_format) {
  ...
  // After the tonemap's, because that pass is what composites what this one produces - and not
  // fatal, for the reason none of the five above are: a device that cannot build it loses bloom and
  // nothing else, since `RecordBloom` and the composite both gate on `BloomPassReady`. Unlike the
  // tonemap's, this failure needs no special reporting: `render.bloom` still reads back as asked and
  // `render.status` says UNAVAILABLE in words.
  if (!CreateBloomPass(width, height)) {
    DebugWrite("gkplus: bloom pass unavailable\n");
    DestroyBloomPass();
  }
...
void RecordTonemap(void *command_buffer, uint64_t dest_view, uint32_t source_width,
  ...
  // The bloom composite (4.99). **Gated on `BloomRanThisFrame` and not on `Bloom()`**, which is the
  // one thing here that has to be got right: the knob is a request, and a layer whose two passes did
  // not run this frame holds either the last frame's blur or nothing at all. That happens on the
  // frame a knob is switched on, on any frame where the colour format is still the 8-bit one, and
  // forever on a device where the pass did not build - so asking the flag that means "the passes
  // ran" is what makes the composite right in all three cases rather than in the common one.
  //
  // The texture index is filled either way, and is always valid: the images live as long as the
  // targets do, and a layer the shader skips never fetches through it.
  //
  // LINEAR and CLAMP, acquired the way the shadow passes acquire theirs - in D3D enums, which is
  // what `AcquireSampler` speaks. It has to be linear: these layers are 60 to 240 lines tall and a
  // point fetch stretched over the frame is visibly blocky, which is the opposite of what a glow is
  // for.
  push.bloom_sampler = AcquireSampler(2, 2, 0, 3, 3);
  for (uint32_t layer = 0; layer < kBloomLayers; ++layer) {
    push.bloom_texture[layer] = BloomSlot(layer, 0);
    const bool live = BloomRanThisFrame && BloomLayers[layer].blend != BloomBlend::Off;
    push.bloom_blend[layer] =
        live ? static_cast<uint32_t>(BloomLayers[layer].blend)
             : static_cast<uint32_t>(BloomBlend::Off);
    push.bloom_intensity[layer] = BloomLayers[layer].intensity;
  }
...
// --- bloom (§4.99) -----------------------------------------------------------------------------
//
// See SetBloom in VkDraw.h. The knobs are stores and take effect on the next frame recorded; unlike
// `render.hdr` and `render.msaa` none of them can invalidate a pipeline, because the layers' format
// and the kernel's width are both decided at record time rather than baked into one.

void SetBloom(bool on) {
  // The env var is consumed here as well as at bring-up, for the reason `SetHdr` gives: reading it
  // after assigning would put the environment's value back over the caller's.
  ReadBloomEnvOnce();
  BloomEnabled = on;
}
...
void SetBloomLayer(uint32_t index, const BloomLayer &layer) {
  if (index >= kBloomLayers) {
    return;
  }
  BloomLayers[index] = layer;
  // Clamped on the way in rather than at every reader. A negative radius would make `BloomBlurTaps`
  // return its floor and the shader divide by a guarded epsilon - correct, but it would read back as
  // the value asked for and behave as something else, which is the shape of defect this codebase
  // keeps finding. Threshold and intensity are deliberately NOT clamped above: an over-range
  // intensity is a legitimate look, and a threshold of 40 is a legitimate way to quieten a layer
  // without changing its blend.
  BloomLayers[index].radius = (std::max)(0.0f, BloomLayers[index].radius);
  BloomLayers[index].knee = (std::max)(0.0f, BloomLayers[index].knee);
  BloomLayers[index].intensity = (std::max)(0.0f, BloomLayers[index].intensity);
}

BloomLayer BloomLayerAt(uint32_t index) {
  return index < kBloomLayers ? BloomLayers[index] : BloomLayer{};
}
...
void RecordBloom(void *command_buffer) {
  auto cmd = static_cast<VkCommandBuffer>(command_buffer);
  BloomLayersThisFrame = 0;
  BloomRanThisFrame = false;
  ReadBloomEnvOnce();
  // **`ColourFormat` and not `Hdr()`**, which is the same distinction the tonemap's `flags` makes:
  // the knob is the request and the format is the answer, and this pass reads the float target
  // itself. Under the 8-bit target there is nothing over 1 in it to find, so running would cost six
  // passes to extract black.
  if (!Ready || !BloomPassReady || !BloomEnabled || cmd == VK_NULL_HANDLE ||
      ColourFormat != kHdrFormat || BloomSourceHeight == 0) {
    return;
  }
  auto set = reinterpret_cast<VkDescriptorSet>(BindlessDescriptorSet());
  ...
  const uint32_t sampler = AcquireSampler(2, 2, 0, 3, 3);
  vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, BloomLayout, 0, 1, &set, 0,
                          nullptr);

  for (uint32_t layer = 0; layer < kBloomLayers; ++layer) {
    // An `Off` layer records nothing at all - not a cheap pass, not a cleared image. That is what
    // makes the per-layer switch free, and it is why the composite has to ask the same question
    // rather than trusting the image to be black.
    if (BloomLayers[layer].blend == BloomBlend::Off || BloomExtent[layer].width == 0) {
      continue;
    }
    const VkExtent2D extent = BloomExtent[layer];
    BloomPushConstants push = {};
    push.dest_width = extent.width;
    push.dest_height = extent.height;
    push.sampler_index = sampler;

    // The extract: the full-resolution float target down to this layer's size, thresholded on the
    // way. It reads `kTonemapSourceSlot` - the same image the tonemap reads, which the caller has
    // already made readable for it, which is why this pass needs no barrier of its own on it.
    push.source_texture = kTonemapSourceSlot;
    push.taps = BloomExtractTaps(BloomSourceHeight, extent.height);
    push.threshold = BloomLayers[layer].threshold;
    push.knee = BloomLayers[layer].knee;
    BloomBarrier(cmd, BloomImage[layer][0], true);
    RecordBloomStep(cmd, BloomExtractPipeline, BloomView[layer][0], extent, push);

    // ... then the separable blur: horizontally into buffer 1 and vertically back into buffer 0, so
    // the finished layer is always in buffer 0 (see BloomSlot in src/VkResources.h).
    const float sigma = BloomSigma(layer);
    push.sigma = sigma;
    push.taps = BloomBlurTaps(sigma);
    for (uint32_t direction = 0; direction < 2; ++direction) {
      const uint32_t source = direction == 0 ? 0u : 1u;
      const uint32_t dest = direction == 0 ? 1u : 0u;
      BloomBarrier(cmd, BloomImage[layer][source], false);
      BloomBarrier(cmd, BloomImage[layer][dest], true);
      push.source_texture = BloomSlot(layer, source);
      push.direction = direction;
      RecordBloomStep(cmd, BloomBlurPipeline, BloomView[layer][dest], extent, push);
    }
    // ... and the finished layer becomes readable for the tonemap.
    BloomBarrier(cmd, BloomImage[layer][0], false);
    ++BloomLayersThisFrame;
  }
  BloomRanThisFrame = BloomLayersThisFrame != 0;
}
...
namespace {
// The twelve departures, as one value that can be snapshotted and compared. The defaults here are
...
struct DepartureSet {
  ...
  bool hdr = false;
  // In the set for the plainest reason of all, and unlike `hdr`'s five sub-knobs: the original could
  // not do this at all. The per-layer thresholds, radii, blends and intensities stay out of it for
  // exactly the reason those five do - they describe how bloom looks, and with `bloom` off there is
  // no bloom for them to describe, so restoring them would put back values that were already inert
  // and would make a look the user chose part of what "stock" means.
  bool bloom = false;
...
DepartureSet CurrentDepartures() {
  ...
  set.hdr = Hdr();
  // `Bloom()` and not `BloomRanThisFrame`, for the reason the two comments above give twice over:
  // what is snapshotted is the request. The flag is false on a device with no pass, on every frame
  // under an 8-bit target, and on any frame where all three layers are off - so restoring through it
  // would switch a knob off that the user had switched on.
  set.bloom = Bloom();
  return set;
}
...
bool ResizeDraw(uint32_t width, uint32_t height) {
  ...
  // right for the extent it was recorded at. Same failure rule as above: the pass goes, not the
  // renderer.
  if (BloomPassReady && !CreateBloomTargets(width, height)) {
    DebugWrite("gkplus: bloom targets could not be resized\n");
    DestroyBloomPass();
  }
...
std::string FormatDrawStats() {
  ...
  if (Hdr() || ColourFormat == kHdrFormat) {
    ...
    // Nested inside the HDR block on purpose, and it is the one report line here whose PLACEMENT is
    // the statement: bloom cannot do anything without a float target, so printing it beside a frame
    // that has none would invite exactly the reading `DescribeBloom` spells out as INERT. Inside, it
    // is only ever seen next to the pipeline it belongs to. Printed whether it is on or off, for the
    // reason the light-sum line above is: it is a departure, so a reader comparing a shot against
    // real D3D8 has to know whether it ran.
    out += DescribeBloom();
  }
