...
`JSCFunctionListEntry` table against its interface, both directions, over 36 namespaces, and takes
...
### Checking this file's own counts

Every roster and count in this file is hand-maintained and has drifted before - four at once, most
recently (a `Subsystems` member that does not exist, one that was missing, a `sync = false` count of
two that was one, and a namespace count of 25 that was 36). They are all one command each, so
re-derive rather than trust:

```bash
sed -n '/Namespaces\[\]/,/^};/p' src/JsGk.cpp | grep -oE '"\w+"\s*,' | wc -l  # gk namespaces (27)
sed -n '/Knobs\[\]/,/^};/p' src/RenderSettings.cpp | grep -c '\.name = '    # persisted knobs (79)
sed -n '/Knobs\[\]/,/^};/p' src/RenderSettings.cpp | grep -c '\.sync = false'  # (1)
sed -n '/Knobs\[\]/,/^};/p' src/RenderSettings.cpp | grep '\.env = ' | grep -v nullptr  # (4)
grep -c '^GK_ACTOR_CLASS' src/ActorClasses.inc.h   # (15)
grep -c '^GUNLOK_MENU' src/Menus.inc.h             # (36)
python3 types/check-surface.py                     # prints its own namespace count (36)
sed -n '/struct Subsystems/,/};/p' src/entry.cpp   # the roster, in construction order
```

**Prefer a derivable count to a written one, and where a list only ever grows, do not number it** -
`blender/CLAUDE.md` leaves its design list unnumbered for exactly this reason, having watched the
count go stale. A `*System` class that exists is `grep -rhoE 'class [A-Za-z]+System' src/*.h`; a
name in this file that greps to nothing in `src/` is a defect in this file.
...
`ActorArgSystem`, `ScriptSystem`.
`src/ImageCodec` is deliberately **not** here — it registers a codec rather than installing a
detour, from `FileHookSystem`'s first intercepted open (see its row under Other).
...
**One knob is restored but never written back**, flagged `sync = false`: `tess.enabled`, which
reads back false on a device with no `tessellationShader` however it was set. `msaa` has the same
*problem* — the effective count is clamped to what the device offers, which is why it is read
through `MsaaWanted()` — but reaches it by the **other** mechanism: it carries
`env = "GKPLUS_VK_MSAA"`, and an env-flagged knob is skipped in both directions. Persisting either
...
- `ghidra_analysis_notes.md` - the reverse-engineering traps and the Ghidra MCP mechanics: what a `.text` sweep can and cannot prove, `RET`-form as the ground truth for calling convention *and* arity, the two parallel class trees, jump-table overrides, the four decompiler artifacts that each flipped a return-type verdict, and the read-only-vs-read-write context rules including snapshot staleness. **Read before concluding anything negative about the binary**
...
### Analysis Traps and Ghidra MCP mechanics

**`ghidra_analysis_notes.md`** - the ~40 accumulated traps of reverse engineering this binary
(what a `.text` sweep can and cannot prove, `RET`-form as the ground truth for calling convention
and arity, the two parallel class trees, jump-table overrides, the decompiler artifacts that each
flipped a verdict) and the mechanics of driving Ghidra through the Jython MCP (read-only vs
read-write contexts, snapshot staleness, `save_program`, the batching and exception rules). Moved
out of this file for the same reason `address_map.md` was: it is looked up during binary work, not
carried into every session.

Read it before concluding anything **negative** - that a field has no writer, a function no caller,
a signature no arguments. Nearly every entry in it is such a conclusion that turned out to be
wrong.
...
Before wrapping any function in it, read the `RET`-form and register-argument warnings in
`ghidra_analysis_notes.md` - a wrong calling convention or arity drifts ESP and faults somewhere
