...
ARG_PATTERNS = [
    ...
    (r"js_to_ImVec2\(ctx,\s*argv\[{i}\]", "Vec2"),
    (r"js_to_ImVec4\(ctx,\s*argv\[{i}\]", "Vec4"),
...
def options_shape(body, index):
    ...
    conversions = [
        ...
        (r"js_to_ImVec2\(ctx,\s*%s", "Vec2"),
        (r"js_to_ImVec4\(ctx,\s*%s", "Vec4"),
    ...
    for var, key in reads:
        ...
        if ts == "any":
            ...
            components = set(re.findall(
                ...
                body))
            if {"x", "y"} <= components:
                ts = "Vec4" if {"z", "w"} <= components else "Vec2"
...
def main():
    ...
    out.append("//")
    out.append("// Types for the \"ImGui\" module registered by src/Script.cpp")
    out.append("// (imgui-quickjs/imgui-quickjs.cpp). The same object is passed")
    out.append("// to the entry module's draw_gui export.")
    out.append("//")
    ...
    out.append("")
    out.append("declare module \"ImGui\" {")
    out.append("  /** An ImVec2, as {x, y}. */")
    out.append("  export interface Vec2 {")
    out.append("    x: number;")
    out.append("    y: number;")
    out.append("  }")
    out.append("")
    out.append("  /** An ImVec4 / ImColor, as {x, y, z, w}. */")
    out.append("  export interface Vec4 {")
    out.append("    x: number;")
    out.append("    y: number;")
    out.append("    z: number;")
    out.append("    w: number;")
    ...
    for kind, name, impl, entry_section in entries:
        ...
        out.append("  export function %s(%s): %s;" % (name, ", ".join(params), ret))
